...
func _process(delta):
	...
	for weapon in player_sprite.weapon_sprites + enemy_sprite.weapon_sprites:
		var last_tick = weapon.get_meta("last_activation", 0)
		var next_tick = weapon.get_meta("next_activation", 0)
		
		var total_duration = (next_tick - last_tick) * seconds_per_tick
...
func get_ticks_until_next_fire(weapon: Dictionary) -> int:
	...
	for i in range(battle_timeline.size()):
		...
		if event.weapon == weapon:
			...
			return battle_timeline.size() - (cT - j)
